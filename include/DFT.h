...
Mat myNormalize(Mat src)
{
	Mat dst;
	src.copyTo(dst);
	normalize(dst, dst, 0, 255, NORM_MINMAX); // 픽셀값 0~255에 대응되도록 영상을 늘려줌
	dst.convertTo(dst, CV_8UC1);				//32F 영상을 8Bit 1channel 영상으로 변환
...
Mat getMagnitude(Mat complexImg)
{
	...
	Mat magImg;
	magnitude(planes[0], planes[1], magImg);	//Magnitude 계산
	magImg += Scalar::all(1);					//상수 1 덧셈
	log(magImg, magImg);
...
Mat getPhase(Mat complexImg)
{
	...
	Mat phaseImg;
	phase(planes[0], planes[1], phaseImg);	//Phase 계산
